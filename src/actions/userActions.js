...
export const getLoggedUserStatus = () => async (dispatch) => {
  try {
    const { data } = await axios.get(`${domain}/api/v1/users/loggedInUser`);
    dispatch({ type: USER_LOGIN_SUCCESS, payload: data });
    localStorage.setItem("userInfo", JSON.stringify(data.data));
  } catch (error) {}
};
...
export const listUsers = (pageNumber) => async (dispatch) => {
  try {
    ...
    const { data } = await axios.get(
      `${domain}/api/v1/users/?page=${pageNumber}`
    );
...
export const deleteUser = (id) => async (dispatch) => {
  try {
    ...
    await axios.delete(`${domain}/api/v1/users/${id}`);
...
export const forgotPassword = (email) => async (dispatch) => {
  try {
    ...
    const config = { headers: { "Content-Type": "application/json" } };
    await axios.post(`${domain}/api/v1/users/forgotPassword`, email, config);
...
export const confirmEmail = (confirmToken) => async (dispatch) => {
  try {
    ...
    const config = { headers: { "Content-Type": "application/json" } };
    await axios.post(
      `${domain}/api/v1/users/confirmEmail/${confirmToken}`,
      config
    );
...
export const resendConfirmEmail = () => async (dispatch) => {
  try {
    ...
    const config = { headers: { "Content-Type": "application/json" } };
    await axios.post(`${domain}/api/v1/users/resendConfirmEmail/`, config);
