    ...
    try {
      ...
      if (category == "all" || category == "") {
        const { data } = await axios.get(
          `${domain}/api/v1/products/?keyword=${keyword}&page=${pageNumber}`
        );
        dispatch({ type: PRODUCT_LIST_SUCCESS, payload: data });
      } else {
        const { data } = await axios.get(
          `${domain}/api/v1/products/?keyword=${keyword}&category=${category}&page=${pageNumber}`
        );
        dispatch({ type: PRODUCT_LIST_SUCCESS, payload: data });
      }
    } catch (error) {
...
export const listProductDetails = (id) => async (dispatch) => {
  try {
    ...
    const { data } = await axios.get(`${domain}/api/v1/products/${id}`);
...
export const deleteProduct = (id) => async (dispatch) => {
  try {
    ...
    await axios.delete(`${domain}/api/v1/products/${id}`, config);
...
export const updateProduct = (product, id) => async (dispatch) => {
  try {
    ...
    if (!(Object.entries(product.image).length === 0))
      formData.append("image", product.image);
    if (product.images)
      product.images.map((product) => formData.append("images", product));
...
export const createProductReview = (productId, review) => async (dispatch) => {
  try {
    ...
    const config = { headers: { "Content-Type": "application/json" } };
    const { data } = await axios.post(
      `${domain}/api/v1/products/${productId}/reviews`,
      review,
      config
    );
...
export const listTopProducts = () => async (dispatch) => {
  try {
    ...
    const { data } = await axios.get(
      `${domain}/api/v1/products/top-3-products/`
    );
